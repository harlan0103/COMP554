...
{
	for (int iterate_num = 0; iterate_num < 10; iterate_num ++) {
		/*** record our result to csv file ***/
		FILE *fp;
		char *s1 = "with_shuffle_results/result";
		char s2[3]; s2[0]='\0';
		sprintf(s2, "%d", iterate_num + 1); 
		char *s3 = ".csv";
		char *file = malloc(strlen(s1) + strlen(s2) + strlen(s3) + 1);//+1 for the zero-terminator
		...
		if ((fp = fopen(file, "w+")) < 0) {
			printf("open the testCSV error\n");
		}

		/*** This is the start of the variables you can play with... ***/
		int size = 1024;			/* The minimum cache size tested for. */
		int maxSize = pow(2, 25);	/* maximum cache size tested for is 256 MB = 8 bytes per pointer * 2^25 ints */
